 ...
 * <domaincheckpoint> element, on a running @domain.
 ...
 * VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE.  Alternatively, if @flags
 * includes VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE_LIST (which requires
 * VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE and is incompatible with
 * VIR_DOMAIN_CHECKPOINT_CREATE_CURRENT), and the domain has no
 * existing checkpoint metadata, then @xmlDesc is parsed as a
 * top-level <checkpoints> element with an optional current='name'
 * attribute, and containing one or more <domaincheckpoint> children
 * (as produced by virDomainGetXMLDesc() with the flag
 * VIR_DOMAIN_XML_CHECKPOINTS), to do a bulk redefine of all
 * checkpoints at once (it is unspecified which of the redefined
 * checkpoints will be used as the return value on success).  Not all
 * hypervisors support these flags.
 *
                             ...
                             unsigned int flags)
{
    ...
    VIR_REQUIRE_FLAG_GOTO(VIR_DOMAIN_CHECKPOINT_CREATE_CURRENT,
                          ...
                          error);
    VIR_REQUIRE_FLAG_GOTO(VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE_LIST,
                          VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE,
                          error);
    ...
    VIR_EXCLUSIVE_FLAGS_GOTO(VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE,
                             ...
                             error);
    VIR_EXCLUSIVE_FLAGS_GOTO(VIR_DOMAIN_CHECKPOINT_CREATE_REDEFINE_LIST,
