...
int
virDomainCheckpointObjListParse(const char *xmlStr,
                                ...
                                unsigned int flags)
{
    ...
    if (checkpoints->metaroot.nchildren || *current_check) {
        virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                       _("bulk define of checkoints only possible with "
    ...
    if (!virXMLNodeNameEqual(root, "checkpoints")) {
        virReportError(VIR_ERR_XML_ERROR,
                       _("unexpected root element <%s>, "
                         "expecting <checkpoints>"), root->name);
        goto cleanup;
    }
    ctxt = xmlXPathNewContext(xml);
    if (ctxt == NULL) {
    ...
    if ((n = virXPathNodeSet("./domaincheckpoint", ctxt, &nodes)) < 0)
        goto cleanup;
    if (!n) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("expected at least one <domaincheckpoint> child"));
        goto cleanup;
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        def = virDomainCheckpointDefParseNode(xml, nodes[i], caps, xmlopt,
                                              flags);
        if (!def)
            goto cleanup;
        if (!(chk = virDomainCheckpointAssignDef(checkpoints, def))) {
            virDomainCheckpointDefFree(def);
            goto cleanup;
        }
        /* Sanity checking, similar to virDomainCheckpointRedefinePrep */
        if (def->dom) {
            if (memcmp(def->dom->uuid, domain_uuid, VIR_UUID_BUFLEN)) {
                ...
                virReportError(VIR_ERR_INVALID_ARG,
                               _("definition for checkpoint %s must use "
                                 "uuid %s"), def->name, uuidstr);
                goto cleanup;
            }
    ...
    if (virDomainCheckpointUpdateRelations(checkpoints) < 0) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("<checkpoints> contains inconsistent parent-child "
                         "relationships"));
        goto cleanup;
    }

    if (current) {
        if (!(*current_check = virDomainCheckpointFindByName(checkpoints,
                                                             current))) {
            virReportError(VIR_ERR_NO_DOMAIN_CHECKPOINT,
                           _("no checkpoint matching current='%s'"), current);
            goto cleanup;
        }
        (*current_check)->def->current = true;
    }

    ret = 0;
 cleanup:
    if (ret < 0) {
        /* There were no checkpoints before this call; so on error, just
         * blindly delete anything created before the failure. */
        virHashRemoveAll(checkpoints->objs);
        checkpoints->metaroot.nchildren = 0;
        checkpoints->metaroot.first_child = NULL;
    }
    VIR_FREE(current);
    xmlXPathFreeContext(ctxt);
    xmlFreeDoc(xml);
                                     ...
                                     unsigned int flags)
{
    ...
    if (!(flags & VIR_DOMAIN_CHECKPOINT_FORMAT_NO_DOMAIN) &&
        virDomainDefFormatInternal(buf, def->dom, &data, domainflags,
                                   xmlopt) < 0)
        goto error;
                             ...
                             unsigned int flags)
{
    ...
    virCheckFlags(VIR_DOMAIN_CHECKPOINT_FORMAT_SECURE |
                  ...
                  VIR_DOMAIN_CHECKPOINT_FORMAT_INTERNAL, NULL);
...
struct virDomainCheckpointFormatData {
    virBufferPtr buf;
    virCapsPtr caps;
    virDomainXMLOptionPtr xmlopt;
    unsigned int flags;
...
static int
virDomainCheckpointFormatOne(void *payload,
                             const void *name ATTRIBUTE_UNUSED,
                             void *opaque)
{
    virDomainCheckpointObjPtr chk = payload;
    struct virDomainCheckpointFormatData *data = opaque;

    return virDomainCheckpointDefFormatInternal(data->buf, chk->def, data->caps,
                                                data->xmlopt, data->flags);
}


int
virDomainCheckpointObjListFormat(virBufferPtr buf,
                                 virDomainCheckpointObjListPtr checkpoints,
                                 virDomainCheckpointObjPtr current_check,
                                 virCapsPtr caps,
                                 virDomainXMLOptionPtr xmlopt,
                                 unsigned int flags)
{
    struct virDomainCheckpointFormatData data = {
        .buf = buf,
        .caps = caps,
        .xmlopt = xmlopt,
        .flags = flags,
    ...
    virBufferAddLit(buf, "<checkpoints");
    if (current_check)
        virBufferEscapeString(buf, " current='%s'",
                              current_check->def->name);
    virBufferAddLit(buf, ">\n");
    virBufferAdjustIndent(buf, 2);
    if (virDomainCheckpointForEach(checkpoints, virDomainCheckpointFormatOne,
                                   &data) < 0) {
        virBufferFreeAndReset(buf);
        return -1;
    }
