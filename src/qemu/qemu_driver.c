                            ...
                            virDomainCheckpointDefPtr def)
{
    ...
    for (i = 0; i < def->ndisks; i++) {
        ...
        if (STRNEQ(disk->bitmap, def->name)) {
            virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
...
/* Struct and hash-iterator callback used when bulk redefining checkpoints */
struct qemuDomainCheckpointBulk {
    virDomainObjPtr vm;
    virQEMUDriverPtr driver;
...
static int
qemuDomainCheckpointBulkRedefine(void *payload,
                                 const void *name ATTRIBUTE_UNUSED,
                                 void *opaque)
{
    virDomainCheckpointObjPtr chk = payload;
    struct qemuDomainCheckpointBulk *data = opaque;

    return qemuDomainCheckpointWriteMetadata(data->vm, chk, data->driver->caps,
                                             data->driver->xmlopt,
                                             data->checkpointDir);
}
                                 ...
                                 unsigned int flags)
{
    ...
    if (virDomainCheckpointListChildrenEnsureACL(checkpoint->domain->conn, vm->def) < 0)
        goto cleanup;

    if (!(chk = qemuCheckObjFromCheckpoint(vm, checkpoint)))
        goto cleanup;

    n = virDomainListAllCheckpoints(vm->checkpoints, chk, checkpoint->domain, chks, flags);
                            ...
                            unsigned int flags)
{
    ...
    checkpoint = virGetDomainCheckpoint(domain, vm->current_checkpoint->def->name);
                        ...
                        virDomainCheckpointObjPtr chk)
{
    ...
    if (chk && def->ndisks != chk->def->ndisks) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
    ...
    for (i = 0; i < def->ndisks; i++) {
        ...
        if (!disk->store) {
            if (chk &&
                chk->def->disks[i].type != VIR_DOMAIN_CHECKPOINT_TYPE_NONE) {
                virReportError(VIR_ERR_OPERATION_UNSUPPORTED,
        ...
        if (chk &&
            chk->def->disks[i].type != VIR_DOMAIN_CHECKPOINT_TYPE_BITMAP) {
            virReportError(VIR_ERR_OPERATION_UNSUPPORTED,
                      ...
                      const char *checkpointXml, unsigned int flags)
{
    ...
    virDomainCheckpointObjPtr chk = NULL;
    virDomainCheckpointObjPtr other = NULL;
    virDomainCheckpointObjPtr parent = NULL;
    ...
    if (checkpointXml) {
        if (!(chkdef = qemuDomainCheckpointDefParseString(driver, caps,
                                                          checkpointXml, 0)) ||
            VIR_STRDUP(suffix, chkdef->name) < 0)
            goto cleanup;
    } else {
    ...
    if (def->incremental) {
        ...
        for (other = vm->current_checkpoint; other;
    ...
    for (i = 0; i < def->ndisks; i++) {
        ...
        if (def->incremental) {
            ...
            for (other = parent ? parent : vm->current_checkpoint; other;
                 other = other->def->parent ?
                     virDomainCheckpointFindByName(vm->checkpoints,
                                                   other->def->parent) : NULL) {
    ...
    */
    ...
    if (chk && qemuDomainCheckpointAddActions(vm, json, parent, chk->def) < 0)
        goto endmon;
 ...
 cleanup:
    virJSONValueFree(arr);
    virDomainCheckpointDefFree(chkdef);
    virCommandFree(cmd);
    VIR_FREE(suffix);
...
static virHypervisorDriver qemuHypervisorDriver = {
    ...
    .domainBackupBegin = qemuDomainBackupBegin, /* 5.2.0 */
    .domainBackupGetXMLDesc = qemuDomainBackupGetXMLDesc, /* 5.2.0 */
    .domainBackupEnd = qemuDomainBackupEnd, /* 5.2.0 */
};
