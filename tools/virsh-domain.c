...
cmdUndefine(vshControl *ctl, const vshCmd *cmd)
{
    ...
    if (!running) {
        ...
        has_checkpoints_metadata
            = virDomainListCheckpoints(dom, NULL,
...
static const vshCmdOptDef opts_dumpxml[] = {
    ...
    {.name = "checkpoints",
     .type = VSH_OT_BOOL,
     .help = N_("include all domain checkpoints in XML dump"),
    },
    {.name = NULL}
};
...
cmdDumpXML(vshControl *ctl, const vshCmd *cmd)
{
    ...
    bool checkpoints = vshCommandOptBool(cmd, "checkpoints");
    ...
    if (checkpoints)
        flags |= VIR_DOMAIN_XML_CHECKPOINTS;
